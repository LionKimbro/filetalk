...
class TestUnregisterComponent(unittest.TestCase):
    ...
    def test_unregister_removes_routes_where_src(self):
        src = declare_component("src")
        dest = declare_component("dest")
        add_route({"src": src, "src-channel": "out", "dest": dest, "dest-channel": "in"})
        unregister_component("src")
        self.assertEqual(routes, [])

    def test_unregister_removes_routes_where_dest(self):
        src = declare_component("src")
        dest = declare_component("dest")
        add_route({"src": src, "src-channel": "out", "dest": dest, "dest-channel": "in"})
        unregister_component("dest")
        self.assertEqual(routes, [])

    def test_unregister_leaves_unrelated_routes(self):
        src = declare_component("src")
        dest = declare_component("dest")
        other = declare_component("other")
        add_route({"src": src, "src-channel": "out", "dest": dest, "dest-channel": "in"})
        unregister_component("other")
...
class TestDelistComponent(unittest.TestCase):
    ...
    def test_removes_routes_where_comp_is_src(self):
        ...
        dest = declare_component("dest")
        add_route({"src": src, "src-channel": "out", "dest": dest, "dest-channel": "in"})
        delist_component(src)
        self.assertEqual(routes, [])

    def test_removes_routes_where_comp_is_dest(self):
        src = declare_component("src")
        dest = declare_component("dest")
        add_route({"src": src, "src-channel": "out", "dest": dest, "dest-channel": "in"})
        delist_component(dest)
        self.assertEqual(routes, [])

    def test_removes_multiple_routes_referencing_comp(self):
        src = declare_component("src")
    ...
    def test_leaves_routes_not_referencing_comp(self):
        a = declare_component("a")
        b = declare_component("b")
        c = declare_component("c")
        add_route({"src": a, "src-channel": "out", "dest": b, "dest-channel": "in"})
        add_route({"src": b, "src-channel": "out", "dest": c, "dest-channel": "in"})
        delist_component(b)
        self.assertEqual(routes, [])  # both routes touched b

    def test_leaves_truly_unrelated_routes(self):
        a = declare_component("a")
        b = declare_component("b")
        c = declare_component("c")
        d = declare_component("d")
        add_route({"src": a, "src-channel": "out", "dest": b, "dest-channel": "in"})
        add_route({"src": c, "src-channel": "out", "dest": d, "dest-channel": "in"})
        delist_component(a)
        self.assertEqual(len(routes), 1)
        self.assertIs(routes[0]["src"], c)

    def test_no_error_on_comp_with_no_routes(self):
        comp = declare_component("lone")
        delist_component(comp)  # must not raise
        self.assertEqual(routes, [])
    ...
    def test_uses_identity_not_equality(self):
        ...
        dest = declare_component("dest")
        add_route({"src": src, "src-channel": "out", "dest": dest, "dest-channel": "in"})
        impostor = _make_component_dict()
        delist_component(impostor)
        self.assertEqual(len(routes), 1)  # real route untouched
